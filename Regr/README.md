...
|-------|---------|---------------|---------|
|  0.019(9)     | NW      | quartic       | 6470.27 |
|   0.03    | NW      | epanenchnekov | 6476.61 |
|   0.05   | LOWESS     | quartic | 7148.13 |
...
| hopt  |  method | core          | SSE     |
|-------|---------|---------------|---------|
|  0.019(9)     | NW      | quartic       | 9035.81 |
|   0.03    | NW      | epanenchnekov | 8483.14 |
|   0.05   | LOWESS     | quartic | 7194.42 |
...
### LASSO (Least Absolute Shrinkage and Selection Operator) Tibshirani

Метод понижения размерности, где минимизация RSS происходит при условии, что сумма абсолютных значений коэффициентов меньше константы. Из-за этих ограничений некоторый коэффициенты получаются равными нулю. 

Сам метод:

![](https://latex.codecogs.com/svg.latex?%7C%7Cy-X%5Ctheta%7C%7C%5E2&plus;%5Ctau%5Csum_%7Bj%3D1%7D%5E%7Bk%7D%7C%5Ctheta_j%7C%5Cto%5Cmin_%7B%5Ctheta%7D.)


Данный метод регуляризации внешне очень похож на гребневую регрессию, но приводит к качественно иному поведению вектора коэффициентов. 
...
![](https://latex.codecogs.com/svg.latex?%5Cleft%5C%7B%5Cbegin%7Bmatrix%7D%20Q%28a%29%20%3D%20%7C%7CF%5Calpha-y%7C%7C%5E2%20%5Crightarrow%20%5Cmin_%7B%5Calpha%7D%5C%5C%20%5Csum_%7Bj%3D1%7D%5En%7C%5Calpha_j%7C%20%5Cleq%20%5Cae%20%5Cend%7Bmatrix%7D%5Cright.)

Если уменьшается ![](https://latex.codecogs.com/svg.latex?%5Cae), (параметр регуляризации) то устойчивость увеличивается и количество ненулевых коэффициентов уменьшается, происходит отбор признаков.

При больших значениях ограничение обращается в строгое неравенство, и решение совпадает с методом наименьших квадратов.

Образно говоря, параметр ![](https://latex.codecogs.com/svg.latex?%5Cae) зажимает вектор коэффициентов, заставляя его отказываться от лишних степеней свободы. Отсюда и название метода.
...
#### Реализация
Датасет - всё тот же boston из sklearn.
...
#### Результаты
Графики собственных значений признаков: 

![](lasso_s.png)

График зависимости значения R^2 Score (коэффициент детерминации 1-rss/tss, одна из возможных оценок) 
...
Оба метода (LASSO, Ridge) успешно решают проблемы мультиколлинеарности, переобучения, и уменьшают разброс коэффициентов. 

Гребневая регрессия использует все признаки, стараясь «выжать максимум» из всей имеющейся информации. 

Лассо производит отбор признаков, что предпочтительнее, когда среди признаков имеются шумовые, или измерения признаков связаны с ощутимыми затратами.
...
Возьмём в пример данную выборку. Линейная модель регрессии не подходит. Делаем предположение, что можем использовать синус для аппроксимации. МНК будет вида:

![](https://latex.codecogs.com/svg.latex?%5Csum%5Cfrac%7B1%7D%7B2%7D%28%5Calpha%5Csin%28bx_i&plus;c%29&plus;d-y_i%29%5E2%20%5Crightarrow%20%5Cmin)
